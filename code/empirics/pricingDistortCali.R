...
#x<-parents[in_cali==1 & only_in_cali!=1 & year==2011 & quarter==1, list(mean_sales = weighted.mean(state_sales_share,tot_rev)), by = "fips_state_code"]
#x[,mean_sales := mean_sales/sum(x$mean_sales)]
#x <- x[taxrates[year==2010, list(fips_state_code = statefips, t_corp = t_corp, t_corp_sales = t_x)], on = "fips_state_code" ]

# read in price data
stores_groups <- readRDS("data/nielsen_extracts/stores_groups_aggregate.Rds")
stores_groups <- stores_groups[year>=2009 & year<=2013]
...
unif <- foreach(cn = category_numbers, .combine = rbind) %do% {
  ...
  unif <- unif[year>=2009 & year<=2013,list(hhi = weighted.mean(hhi,total_revenue)), by = list(year, quarter, group)]
  unif[, list(hhi = mean(hhi)), by = "group"]
}
unif[, quartile :=  cut(hhi, breaks = quantile(hhi, probs = 0:5/5),
                        labels = FALSE, include.lowest = TRUE)]

stores_groups[unif, on = "group", `:=`(hhi_quartile = quartile)]
...
results_group <- foreach(cn = category_numbers, .packages = c('data.table','jtools','fixest', 'zoo'), .combine = rbind) %dopar% {
  products <- stores_groups[group==cn]
  products[, yearqtr := as.yearqtr(paste0(year,"-",quarter))]
  setkeyv(products, c("store_code_uc","yearqtr"))
  products[, price := revenue/quantity]
  products[, lprice := log(price)]
  products[, dlprice_pre := lprice - log(price[year == 2010 & quarter == 4]), .(store_code_uc)]
  ...
  tryCatch(
    {
      reg1 <- feols(dlprice_pre ~ post2011 + only_in_cali + post2011*only_in_cali |  yearqtr, products[fips_state_code==6], cluster = "store_code_uc")
    },
    error=function(cond) {
      reg1 <- NULL
      return(NULL)
  ...
  tryCatch(
    {
      reg2 <- feols(dlprice_pre ~ post2011 + in_cali + post2011*in_cali |  yearqtr, products[fips_state_code!=6], cluster = "store_code_uc")
    },
    error=function(cond) {
      reg2 <- NULL
      return(NULL)
  ...
  tryCatch(
    {
      reg3 <- feols(dlprice_pre ~ post2011 + in_cali + post2011*in_cali |  yearqtr, products[fips_state_code!=6 & zerotax], cluster = "store_code_uc")
    },
    error=function(cond) {
      reg3 <- NULL
      return(NULL)
  ...
  tryCatch(
    {
      reg4 <- feols(dlprice_pre ~ post2011 + in_cali + post2011*in_cali |  yearqtr, products[fips_state_code!=6 & presence_count>=2 & presence_count<=5], cluster = "store_code_uc")
    },
    error=function(cond) {
      reg4 <- NULL
      return(NULL)
  ...
  tryCatch(
    {
      reg2a <- feols(dlprice_pre ~ post2011 + cali_sales_share_pre + post2011*cali_sales_share_pre |  yearqtr, products[fips_state_code!=6], cluster = "store_code_uc")
    },
    error=function(cond) {
      reg2a <- NULL
      return(NULL)
  ...
  tryCatch(
    {
      reg3a <- feols(dlprice_pre ~ post2011 + cali_sales_share_pre + post2011*cali_sales_share_pre |  yearqtr, products[fips_state_code!=6 & zerotax], cluster = "store_code_uc")
    },
    error=function(cond) {
      reg3a <- NULL
      return(NULL)
  ...
  tryCatch(
    {
      reg4a <- feols(dlprice_pre ~ post2011 + cali_sales_share_pre + post2011*cali_sales_share_pre |  yearqtr, products[fips_state_code!=6 & presence_count>=2 & presence_count<=5], cluster = "store_code_uc")
    },
    error=function(cond) {
      reg4a <- NULL
      return(NULL)
  ...
  tryCatch(
    {
      reg5 <- feols(dlprice_pre ~ post2011 + in_cali + post2011*in_cali |  yearqtr, products[fips_state_code!=6 & zerotax & presence_count>=2 & presence_count<=5], cluster = "store_code_uc")
    },
    error=function(cond) {
      reg5 <- NULL
      return(NULL)
  ...
  tryCatch(
    {
      reg6 <- feols(dlprice_pre ~ post2011 + (fips_state_code==6) + post2011*(fips_state_code==6) |  parent_code^yearqtr,
  ...
  tryCatch(
    {
      reg7 <- feols(dlprice_pre ~ post2011 + (fips_state_code==6) + post2011*(fips_state_code==6) |  parent_code^yearqtr,
...
saveRDS(results_group, "reg_results/results_group4.Rds")
